      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            ...
            >
              {/* Drawer Header */}
              <div className="flex items-center justify-between p-5 bg-gradient-to-r from-slate-900 to-slate-800 border-b border-slate-700">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center font-black text-white border-2 border-white/20 text-sm">
                    {user?.fullName?.charAt(0)}
                  </div>
                  <div>
                    <p className="font-black text-sm text-white truncate max-w-[150px]">{user?.fullName}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Hospital Console</p>
                  </div>
                </div>
                <button
                  onClick={() => setMobileMenuOpen(false)}
              ...
              <nav className="flex-1 p-4 space-y-1">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 px-3 mb-2">Hospital Operations</p>
                {[
                  { id: 'operations', label: 'Operations Center', icon: Activity, desc: 'Blood Inventory & Campaigns' },
                  { id: 'donorRegistry', label: 'Smart Match Finder', icon: Users, desc: 'Find Available Donors' },
                  { id: 'browseRequests', label: 'Browse Requests', icon: Heart, desc: 'Regional Blood Requests' }
                ].map((item) => {
                      ...
                      {loadingMatchingDonors ? (
                        ...
                        <div className="space-y-3 max-h-[160px] overflow-y-auto pr-1">
                          {matchingDonors.map((donor) => {
                            ...
                            return (
                              <div key={donor._id} className="p-3 bg-white dark:bg-dark-800/80 rounded-xl border border-slate-100 dark:border-slate-800 flex flex-col gap-2 text-[11px] shadow-sm">
            ...
            >
              <div className="space-y-6">
                <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
                  <div>
                    <span className="text-[10px] font-black tracking-widest uppercase text-slate-400">Hospital Console</span>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-4 flex items-center gap-3">
                ...
                <div className="text-left text-xs truncate max-w-[140px]">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase">Hospital Portal</p>
                </div>
              </div>
            </motion.div>
          </>
