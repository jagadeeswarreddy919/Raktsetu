/**
 * RaktSetu WhatsApp Alert & Gateway Utility
  ...
  try {
    ...
    if (callmebotApiKey) {
      ...
      if (cleanTo !== recipientPhone && callmebotPhone) {
        finalMessage = `[RaktSetu Admin Forwarding Alert]\nForward this OTP to standard user (${cleanTo}):\n\n${message}`;
      }
    ...
    console.log(`${borderChar}  SENDER   : RaktSetu WhatsApp Dispatcher (SIMULATED GATEWAY)       ${borderChar}`);
