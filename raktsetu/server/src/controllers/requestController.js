...
exports.pledgeDonation = async (req, res) => {
  try {
    ...
    if (recipientUser) {
      // 1. Send SMS alert
      try {
        const acceptSms = `❤️ RAKTSETU: Your request for ${request.patientName} was accepted by donor ${donorUser.fullName} (${donorUser.bloodGroup}). Call: ${donorUser.phone}, WA: https://wa.me/${targetWa}, Chat: http://localhost:5173/chat`;
        await sendSMS({
    ...
    if (status === 'Donated') {
      ...
      if (donor) {
        ...
        try {
          ...
          try {
            const thankYouSms = `🎉 RAKTSETU: Thank you, ${donor.fullName}! Your donation for ${request.patientName} has been verified. You've earned 200 Reward Points! ❤️`;
            await sendSMS({
              to: donor.phone,
              message: thankYouSms
            });
