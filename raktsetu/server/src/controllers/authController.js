...
<body>
  <div class="wrapper">
    ...
    <div class="footer">
      <p style="margin: 0;">&copy; ${new Date().getFullYear()} RaktSetu. All rights reserved.</p>
...
exports.mockSendVerification = async (req, res) => {
  try {
    ...
    const emailHtml = getEmailTemplate(
      'Verify Your Account',
      `Thank you for joining RaktSetu. To unlock all features including blood requests and donation scheduling on the registry dashboards, you can verify your email address in two ways:<br><br>` +
    ...
    const info = await sendMail({
      to: email,
      subject: 'Verify your RaktSetu Account',
      html: emailHtml
    });
...
exports.mockVerifyEmail = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase() });
    if (!user) {
      return res.status(404).send(getBeautifulPageHTML('Not Found', '<h1>User Not Found</h1><p>No account associated with this email address exists in the RaktSetu registry.</p>'));
    }
    ...
    const successContent = `
      ...
      <p>Your email address <strong>${email}</strong> has been successfully verified in the RaktSetu registry. You can now use all platform capabilities.</p>
...
exports.mockForgotPassword = async (req, res) => {
  try {
    ...
    const emailHtml = getEmailTemplate(
      'Reset Your Password',
      'We received a request to recover the password associated with your RaktSetu account. To configure a new password, please click the secure link below.',
      recoveryUrl,
    ...
    const info = await sendMail({
      to: email,
      subject: 'Recover your RaktSetu Password',
      html: emailHtml
    });
...
exports.mockResetPassword = async (req, res) => {
  try {
    ...
    const successContent = `
      ...
      <p>Your password has been successfully updated. You can now securely sign in to RaktSetu using your new credentials.</p>
...
exports.adminForgotPassword = async (req, res) => {
  try {
    ...
    const message = `[RaktSetu Security] Your administrative password reset code is: ${otp}. This code is valid for 10 minutes. Please enter this code in the portal to reset your password.`;
...
exports.userForgotPassword = async (req, res) => {
  try {
    ...
    // Send WhatsApp notification
    const message = `[RaktSetu Security] Your password reset verification code is: ${otp}. This code is valid for 10 minutes.`;
    const dispatchResult = await sendWhatsApp({ to: user.phone, message });
...
exports.adminForgotPasswordEmail = async (req, res) => {
  try {
    ...
    const emailHtml = getEmailTemplate(
      ...
      `We received a request to verify your identity for your RaktSetu administrator account.<br><br>` +
    ...
    await sendMail({
      to: user.email,
      subject: '[RaktSetu Security] Administrative Password Recovery OTP',
      html: emailHtml
    });
...
exports.userForgotPasswordEmail = async (req, res) => {
  try {
    ...
    const emailHtml = getEmailTemplate(
      ...
      `We received a request to verify your identity for your RaktSetu account.<br><br>` +
      `Your 6-digit recovery code is: <strong style="font-size: 24px; color: #e11d48; letter-spacing: 2px;">${otp}</strong>.<br><br>` +
    ...
    await sendMail({
      to: user.email,
      subject: '[RaktSetu Security] Password Recovery OTP',
      html: emailHtml
    });
...
exports.adminResendSms = async (req, res) => {
  try {
    ...
    // Dispatch SMS
    const message = `[RaktSetu Security] Your administrative password reset code is: ${otp}. This code is valid for 10 minutes.`;
    await sendSMS({ to: user.phone, message });
