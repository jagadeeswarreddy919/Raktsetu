# RaktSetu Premium PowerShell Launcher Controller
# Designed for deep red/crimson aesthetics, system health checks, and automated starting.

$Host.UI.RawUI.WindowTitle = "RaktSetu Launcher Controller"
Clear-Host
...
$ToolsDir = "$PSScriptRoot\raktsetu\tools"
...
"@ -ForegroundColor $Crimson
    ...
    Write-Host "           RaktSetu Integrated Blood Donation & AI Platform" -ForegroundColor $White
    Write-Host "  ===============================================================" -ForegroundColor $Gray
    Write-Host ""
}
...
$serverRoot = "$PSScriptRoot\raktsetu\server"
$clientRoot = "$PSScriptRoot\raktsetu\client"
...
while ($true) {
    ...
    switch ($selection) {
        "1" {
            Write-Host "`n  Spawning Core RaktSetu Application..." -ForegroundColor $Green
            
            # Start Backend
            $backendArgs = "-NoExit -Command `"cd '$serverRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Backend Server'; npm run dev`""
            Start-Process powershell -ArgumentList $backendArgs -WorkingDirectory $serverRoot
            
            # Start Frontend
            $clientArgs = "-NoExit -Command `"cd '$clientRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Frontend Client'; npm run dev`""
        ...
        "2" {
            ...
            # Start AI/ML Service
            $mlRoot = "$PSScriptRoot\raktsetu\ml-service"
            if (Test-Path "$mlRoot\venv") {
                $mlArgs = "-NoExit -Command `"cd '$mlRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Machine Learning Service'; .\venv\Scripts\activate; python -m app.main`""
            } else {
                $mlArgs = "-NoExit -Command `"cd '$mlRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Machine Learning Service'; python -m app.main`""
            }
            Start-Process powershell -ArgumentList $mlArgs -WorkingDirectory $mlRoot

            # Start Analytics Service
            $analyticsRoot = "$PSScriptRoot\raktsetu\analytics-service"
            ...
            if (Test-Path $jarFile) {
                $analyticsArgs = "-NoExit -Command `"$envSetup cd '$analyticsRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Analytics Service'; java -jar target/analytics-service-0.0.1-SNAPSHOT.jar`""
            } else {
                # Fallback to build with mvnw if available, or try to run maven
                if (Test-Path "$analyticsRoot\mvnw") {
                    $analyticsArgs = "-NoExit -Command `"$envSetup cd '$analyticsRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Analytics Service'; .\mvnw spring-boot:run`""
                } else {
                    $analyticsArgs = "-NoExit -Command `"$envSetup cd '$analyticsRoot'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Analytics Service'; mvn spring-boot:run`""
                }
            }
        ...
        "3" {
            ...
            $dockerArgs = "-NoExit -Command `"cd '$PSScriptRoot\raktsetu'; `$Host.UI.RawUI.WindowTitle = 'RaktSetu Docker Containers'; docker-compose up --build`""
            Start-Process powershell -ArgumentList $dockerArgs -WorkingDirectory "$PSScriptRoot\raktsetu"
            Exit
        }
        "4" {
